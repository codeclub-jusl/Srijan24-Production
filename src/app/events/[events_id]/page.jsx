...
export default () => {
    ...
    return (
        <div className='l'>
            <div id='event-name'>{eventData.eventName}</div>

            <img src={eventData.eventPoster} alt={eventData.eventName} />
            <div id='event-date-prelims'>
                {' '}
                {eventData.eventDate.prelims[0]}{' '}
            </div>
            <div id='event-date-finals'> {eventData.eventDate.finals} </div>
            <div id='event-type'> {eventData.eventType} </div>
            <div id='event-hashtags'>
                {' '}
                {eventData.eventHashtags.join(', ')}{' '}
            </div>
            <div id='event-coordinators'>
                {eventData.eventCoordinators.map((coordinator, index) => (
                    <div key={index}>{coordinator}</div>
                ))}

                <div id='min-members'> {eventData.minMembers} </div>

                <div id='max-members'> {eventData.maxMembers} </div>
                <div id='event-description'>
                    {eventData.eventDescription.map((description, index) => (
                        <div key={index}>{description}</div>
                    ))}
                    <div id='event-rules'>
                        {' '}
                        {eventData.eventRules.join(', ')}{' '}
                    </div>
                    <div id='event-rule-link'></div>
                </div>
            </div>
        </div>
    )
